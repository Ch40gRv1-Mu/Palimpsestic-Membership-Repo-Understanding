"""
End-to-end RESHUFFLING experiment on TinyStories (observational setting).

This is the most complete experiment script. It implements the full pipeline:

  1. TRAIN: Train a small LLaMA model on TinyStories with a known data order
     - First train a "partial" model on the first n_partial_0 samples
     - Then continue training to n_base samples (the "base model" = Alice's model)

  2. FINETUNE: Optionally fine-tune on additional data (simulates Bob modifying Alice's model)

  3. SAMPLE: Generate text from Bob's model (what Alice observes)

  4. SHUFFLE: Create reshuffled control models — retrain from the partial checkpoint
     with DIFFERENT data orderings. The last "shuffle" uses the ORIGINAL order
     (i.e., it IS Alice's model). The others are random reshuffles → null distribution.

  5. TEST: Evaluate all models on the generated text. If Alice's model assigns
     higher likelihood to its own generated text than reshuffled models do,
     that's evidence the text came from Alice's model.

The z-score is: (metric_original - mean_reshuffled) / std_reshuffled
A large positive z-score → the original order matters → evidence of derivation.
"""
...
Z_SCORE_EPS = 1e-6  # small epsilon to avoid division by zero in z-score
...
def run_training(args):
    """Step 1: Train Alice's model with a known data order.

    Training is done in stages to support the reshuffling test:
      - Train on texts[0:n_partial_0] → save "partial model 0" (shared prefix)
      - Train on texts[n_partial_0:...] in intervals → save intermediate partial models
      - Train on texts[...:n_base] → save "base model" (Alice's final model)

    The partial models serve as checkpoints: reshuffled models will diverge
    from one of these checkpoints with different data orderings.
    """
    tokenizer = get_tokenizer()
...
def run_finetuning(args):
    """Step 2: Simulate Bob fine-tuning Alice's base model on new data.

    Fine-tunes on texts[n_base:n_base+n_finetune] — data Alice didn't train on.
    This tests whether the palimpsestic signal survives fine-tuning.
    """
    tokenizer = get_tokenizer()
...
def run_testing(args):
    """Step 5: Compare Alice's model vs reshuffled models on generated text.

    For each shuffle model (including the original-order one as the last),
    evaluate log-likelihood on the generated samples.

    The z-score measures how much better the original-order model fits
    the generated text compared to the reshuffled controls.
    z-score >> 0 → the original training order left a detectable trace.
    """
...
def run_sampling(args):
    """Step 3: Generate text from Bob's (fine-tuned) model. This is what Alice observes."""
...
def run_shuffling(args):
    """Step 4: Create reshuffled control models for the permutation test.

    Starting from the same partial checkpoint, retrain with DIFFERENT random
    data orderings. This creates the null distribution: models that saw the
    same data but in different orders.

    IMPORTANT: The LAST shuffle (i=num_shuffles-1) uses shuffle=False,
    meaning it trains in the ORIGINAL order. This is the model we're testing
    against the reshuffled ones.
    """
    tokenizer = get_tokenizer()
    texts = get_training_texts(args.seed)
...
def experiment_metric(tokenized_text, prediction, tokenized_prompt):
    """Sum of log-probabilities for the COMPLETION part only (excluding the prompt).

    Higher value (less negative) → model assigns higher probability to the completion
    → better "memorization" of this type of text.
    """
...
def main():
    """Orchestrate the full experiment pipeline: train → shuffle → finetune → sample → test."""
